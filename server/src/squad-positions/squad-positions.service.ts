...
export class SquadPositionsService {
  ...
  findByIdOrThrow(id: string, withRelations?: boolean): Promise<SquadPosition> {
    if (!id) throw new BadRequestException();

    return this.squadPositionRepository.findOneOrFail(id, { relations: withRelations ? ['member', 'chapter'] : [] });
  }
  ...
  async create(input: CreateSquadPositionInput) {
  ...
  async update({ id, ...input }: UpdateSquadPositionInput) {
    const squadPosition = await this.findByIdOrThrow(id);
  ...
  async delete(id: string) {
    const squadPosition = await this.findByIdOrThrow(id, true);
